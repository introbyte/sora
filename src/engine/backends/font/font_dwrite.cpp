...
font_init() {
	...
	hr = font_state.dwrite_factory->CreateCustomRenderingParams(gamma, enhanced_contrast, clear_type_level, DWRITE_PIXEL_GEOMETRY_FLAT, DWRITE_RENDERING_MODE_DEFAULT, &font_state.rendering_params);
...
font_glyph_raster(arena_t* arena, font_t* font, f32 size, u32 codepoint) {
	...
	render_target->DrawGlyphRun(draw_pos.x, draw_pos.y, DWRITE_MEASURING_MODE_NATURAL, &glyph_run, font_state.rendering_params, RGB(255, 255, 255), &bounding_box);
