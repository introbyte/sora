...
mesh_load(arena_t* arena,  str_t filepath) {
	...
	u8 splits[] = { '\n' };
	str_list_t lines = str_split(scratch.arena, data, splits, 1);
	...
	vec3_t* positions = (vec3_t*)arena_alloc(scratch.arena, sizeof(vec3_t) * position_count);
	vec2_t* tex_coords = (vec2_t*)arena_alloc(scratch.arena, sizeof(vec2_t) * tex_coord_count);
	vec3_t* normals = (vec3_t*)arena_alloc(scratch.arena, sizeof(vec3_t) * normal_count);
	...
	printf("[info] successfully loaded mesh: '%.*s'\n", filename.size, filename.data);
