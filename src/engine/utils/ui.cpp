...
ui_context_create(os_handle_t window, gfx_handle_t renderer) {
    ...
    //context->color_accent_default_node.v = color(0xf5b500ff); // yellow
    context->color_accent_default_node.v = color(0x3BD3B7ff); // cyan
...
ui_float_edit(str_t label, f32* value, f32 delta, f32 min, f32 max) {
    ...
    // hide if not hovered
    if (show_arrows) {
        frame_flags |= ui_frame_flag_draw_text;
    }
