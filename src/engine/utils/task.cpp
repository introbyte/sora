...
task_init(u32 worker_thread_count, u32 fiber_count) {
    ...
    // create queues
    {
        task_state.mutex = os_mutex_create();
...
task_release() {
    ...
    // release queues
    for(i32 i = 0; i < task_priority_count; i++) {
        task_queue_release(task_state.task_queues[i]);
    }
    task_queue_release(task_state.counter_free_indices);
    task_queue_release(task_state.fiber_sleeping);
    task_queue_release(task_state.fiber_free_indices);
    
    // release mutex
    os_mutex_release(task_state.mutex);
...
task_run(task_desc_t* task_descs, u32 tasks_count) {
    ...
    os_mutex_lock(task_state.mutex);
    ...
    os_mutex_unlock(task_state.mutex);
